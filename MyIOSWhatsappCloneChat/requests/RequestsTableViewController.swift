...
class RequestsTableViewController: UITableViewController {
    ...
    func initFireBase(){
        ...
        self.mRef.child("chat_requests").child(self.userID!).child(uid).observe(DataEventType.value, with: { (dataSnapShot) in
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.acceptBtn.addTarget(self, action: #selector(acceptBtnSelected), for: .touchUpInside)
        
        cell.rejectBtn.tag = indexPath.row
        cell.rejectBtn.addTarget(self, action: #selector(rejectBtnSelected), for: .touchUpInside)
    ...
    @objc func acceptBtnSelected(sender: UIButton, path:Int){
        
        var index:Int = sender.tag.self
        
        var contactSaved:[String:String] = ["contact":"saved"]
       
        
        //write sent entry to the current user id -> added user -> sent
        self.mRef.child("chat_requests").child(self.userID!).child(allUserIDs[index]).removeValue()
        //writes receive entry to the added user - > current user id -> received
        self.mRef.child("chat_requests").child(allUserIDs[index]).child(self.userID!).removeValue()
        
          self.mRef.child("contacts").child(allUserIDs[index]).child(self.userID!).updateChildValues(contactSaved)
         self.mRef.child("contacts").child(self.userID!).child(allUserIDs[index]).updateChildValues(contactSaved)
